...
const {TagsModel} = require('./symbols-model');
...
module.exports = {
  config: {
    ...
    showVariables: {
      type: 'boolean',
      default: true,
      description: 'Include variables in coding outlines'
    },
    spacesPerTab: {
      type: 'integer',
      description: 'For coding outlines, how many spaces should count for a tab?',
      default: 2,
      minimum: 1,
      maximum: 8
    }
  ...
  activate() {
    ...
    this.subscriptions.add(atom.config.observe('document-outline.showVariables', () => {
      this.update(atom.workspace.getActiveTextEditor());
    }));

    this.subscriptions.add(atom.config.observe('document-outline.spacesPerTab', () => {
      this.update(atom.workspace.getActiveTextEditor());
    }));
  ...
  update(editor) {
    ...
    if( !this.has_files_tags && this.docModel ) {
      this.has_files_tags = this.docModel.getFTL();
    }

    if (this.view) {
      if (this.docModel) {
        var fn = editor.getPath();
        var buffer_length = editor.getBuffer().getText().length;
        var outline;

        if( (this.has_files_tags) && ( fn in this.myCtrls ) && ( this.myCtrls[fn][0] == buffer_length ) ) {
          outline = this.myCtrls[fn][1];
          this.view.update({outline, editor});
          return;
        }

        outline = this.docModel.getOutline();
        if (outline) {
          if( this.has_files_tags ) {
            //console.log("(saving outline from update: '" + fn + "')");
            this.myCtrls[fn] = [ buffer_length, outline ];
          }
          this.view.update({outline, editor});
        }
      } else {
  ...
  getDocumentModel(editor) {
    ...
    var fn = editor.getPath();
    /*if( fn in this.myFiles ) { -- I don't know why, but this doesn't work. We won't need it, though.
      console.log("(preloaded filebuffer '" + fn + "')");
      return this.myFiles[fn];
    }*/
    ...
    //console.log("(saving buffer '" + fn + "')");
    //this.myFiles[fn] = docModel;
    return docModel;
  },
  ...
  provideOutlines() {
    ...
    return {
      ...
      getOutline: editor => {
        ...
        if( !this.has_files_tags && docModel && docModel.getFTL ) {
          //this.files_tags = docModel.getFTL();
          this.has_files_tags = docModel.getFTL();//true;
          //console.log("GetFTL: " + this.has_files_tags);
        }

        if( this.has_files_tags && fn in this.myCtrls ) {
          if( this.myCtrls[fn][0] == buffer_length ) {
            //console.log("(preloaded outline: '" + fn + "')");
            return {outlineTrees: this.myCtrls[fn][1]};
          }
        }

        var myoutline = docModel.getOutline();

        if( this.has_files_tags ) { // files_tags['ctags_read_complete'] == true ) {
          this.myCtrls[fn] = [ buffer_length, myoutline ];
          //console.log("(saving outline: '" + fn + "')");
        }
        return {outlineTrees: myoutline };
      }
    };
  }
};
